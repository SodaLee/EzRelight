...
class StableDiffusionXLControlNeXtImg2ImgPipeline(
    DiffusionPipeline,
    ...
    def __init__(
        ...
        controlnext: Union[ControlNetModel, List[ControlNetModel], Tuple[ControlNetModel], MultiControlNetModel],
    ...
    def encode_prompt(
        ...
        if prompt_embeds is None:
            ...
            if light_image is not None:
                b, _, f, _, _ = light_image.shape
                light_image = rearrange(light_image, "b c f h w -> (b f) c h w")
    ...
    def prepare_latents(
        ...
        if image.shape[1] == 4:
            ...
            if isinstance(generator, list) and len(generator) != batch_size:
                ...
                init_latents = rearrange(init_latents, "(b f) c h w -> b c f h w", f=f)
            else:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        b, _, f, _, _ = image.shape
        image = rearrange(image, "b c f h w -> (b f) c h w")
        control_image = rearrange(control_image, "b c f h w -> (b f) c h w")
        control_image_2 = rearrange(control_image_2, "b c f h w -> (b f) c h w")
        ...
        control_image = rearrange(control_image, "(b f) c h w -> b c f h w", f=f)
        control_image_2 = rearrange(control_image_2, "(b f) c h w -> b c f h w", f=f)
